...
namespace Cubed.Data.Types {
	...
	public class Ray {
		...
		/// </summary>
		public Ray() : this(Vector3.Zero, Vector3.Zero, -1f, (Entity[])null) { }

		/// <summary>
		/// Создание луча со списком объектов
		/// </summary>
		/// <param name="entities">Список объектов для проверки</param>
		public Ray(Entity[] entities) : this(Vector3.Zero, Vector3.Zero, entities) { }

		/// <summary>
		/// Создание луча со списком объектов сцены
		/// </summary>
		/// <param name="scene">Сцена для сбора объектов</param>
		public Ray(Scene scene) : this(Vector3.Zero, Vector3.Zero, scene) { }
		...
		public Ray(Vector3 position, Vector3 direction) : this(position, direction, -1f, (Entity[])null) { }
		...
		/// <param name="position">Позиция</param>
		/// <param name="direction">Направление</param>
		public Ray(Vector3 position, Vector3 direction, float length) : this(position, direction, length, (Entity[])null) { }
		...
		/// <param name="direction">Направление</param>
		/// <param name="entities">Список объектов для проверки</param>
		public Ray(Vector3 position, Vector3 direction, Entity[] entities) : this(position, direction, -1f, entities) { }
		...
		/// <param name="direction">Направление</param>
		/// <param name="scene">Сцена для сбора объектов</param>
		public Ray(Vector3 position, Vector3 direction, Scene scene) : this(position, direction, -1f, scene) { }
		...
		/// <param name="entities">Список объектов для проверки</param>
		public Ray(Vector3 position, Vector3 direction, float length, Entity[] entities) {
			Position = position;
			Direction = direction;
			Length = length;
			Entities = entities;
		...
		/// <param name="length">Длина</param>
		/// <param name="scene">Сцена для сбора объектов</param>
		public Ray(Vector3 position, Vector3 direction, float length, Scene scene) {
			Position = position;
			Direction = direction;
			Length = length;
			Scene = scene;
		...
		public void FromCamera(Camera cam, float x, float y) {
			Position = cam.ScreenToPoint(new Vector3(x, y, 0f));
			Direction = cam.ScreenToPoint(new Vector3(x, y, 1f)) - Position;
		}
