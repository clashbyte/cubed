	...
	public class MeshComponent : EntityComponent, IRenderable {
		...
		public virtual Vector3[] Normals {
			get {
				...
				if (norms != null) {
					Vector3[] na = new Vector3[norms.Length / 3];
		...
		/// </summary>
		public virtual Vector2[] TexCoords {
			get {
				float[] tcrd = SearchTexCoords();
				if (tcrd != null) {
					Vector2[] ca = new Vector2[tcrd.Length / 2];
		...
		/// <summary>
		/// Отрисовать сетку меша в режиме наложения
		/// </summary>
		...
		protected virtual void LegacyRender() {
			...
			if (va != null && ia != null) {
				if (va.Length > 0 && ia.Length > 0) {
					...
					// Отрисовка элементов
					GL.DrawElements(PrimitiveType.Triangles, ia.Length, DrawElementsType.UnsignedShort, ia);

					// Выключение состояний
					GL.Disable(EnableCap.Light0);
					GL.Disable(EnableCap.Lighting);

					// Отрисовка ещё раз, если требуется оверлей
					if (WireframeOverlay) {
						GL.BindTexture(TextureTarget.Texture2D, 0);
						GL.Color3(WireframeColor);
